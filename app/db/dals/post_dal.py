...
def create_post(db: Session, post: post_schemas.PostCreate):
    ...
    db_post.owner_id = post.owner_id
    db.add(db_post)
    db.commit()
    db.refresh(db_post)
    return db_post
...
def get_user_posts(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db.query(post_model.Post).filter(post_model.Post.owner_id == user_id).order_by(post_model.Post.created_on.desc()).offset(skip).limit(limit).all()


def get_all_posts(db: Session, skip: int = 0, limit: int = 100):
    return db.query(post_model.Post).order_by(post_model.Post.created_on.desc()).offset(skip).limit(limit).all()
