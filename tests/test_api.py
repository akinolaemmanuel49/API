...
def test_create_user():
    response = client.post(
        "/users/", json={"username": "testuser0", "email": "testuser0@mail.com", "password": "password"})
    assert response.status_code == 201


def test_create_user_duplicate_email():
    response = client.post(
        "/users/", json={"username": "testuser1", "email": "testuser0@mail.com", "password": "password"})
    assert response.status_code == 400


def test_create_user_duplicate_username():
    response = client.post(
        "/users/", json={"username": "testuser0", "email": "testuser1@mail.com", "password": "password"})
    assert response.status_code == 400


def test_create_user_missing_username_field():
    response = client.post(
        "/users/", json={"email": "testuser1mail.com", "password": "password"})
    assert response.status_code == 422


def test_create_user_missing_email_field():
    response = client.post(
        "/users/", json={"username": "testuser1", "password": "password"})
    assert response.status_code == 422
...
def test_create_user_validate_email():
    response = client.post(
        "/users/", json={"username": "testuser1", "email": "testuser1mail.com", "password": "password"})
    assert response.status_code == 422
